...
public class ImageProcessing {
    ...
    int[][] newTwoD = new int[imageTwoD.length][imageTwoD[0].length];
    for (int i = 0; i < imageTwoD.length; i++) {
      for (int j = 0; j < imageTwoD[i].length; j++) {
        ...
        rgba[1] = 255 - rgba[1];
        rgba[2] = 255 - rgba[2];
        newTwoD[i][j] = getColorIntValFromRGBA(rgba);
      } 
    }
  	return newTwoD;
	}
	...
	public static int[][] shrinkVertically(int[][] imageTwoD) {
		// TODO: Fill in the code for this method
    int[][] newTwoD = new int[imageTwoD.length/2][imageTwoD[0].length];
    for (int i = 0; i < imageTwoD[0].length; i++) {
      for (int j = 0; j < imageTwoD.length-1; j+=2) {
        newTwoD[j/2][i] = imageTwoD[j][i];
      }
    }
		return newTwoD; 
	}
    ...
    for (int i = 0; i < numRectangles; i++) {
      ...
      canvas = paintRectangle(canvas, randWidth, randHeight, randRowPos, randColPos, randomColor);
    }
		return canvas;
	}
	// Utility Methods
